...
namespace GSGUnityUtilities.Runtime
{
    ...
    public class ReferenceManager : Singleton<ReferenceManager>, IReferenceManager
    {
        ...
        // 添加引用
        public virtual void AddReference<T>(T reference) where T : IReference
        {
        ...
        // 获取引用
        public virtual T GetReference<T>() where T : class, IReference
        {
            string key = typeof(T).Name;
        ...
        // 检查是否包含引用
        public virtual bool ContainsReference<T>() where T : class, IReference
        {
            string key = typeof(T).Name;
        ...
        // 移除引用
        public virtual void RemoveReference<T>(T reference) where T : IReference
        {
            string key = typeof(T).Name;
